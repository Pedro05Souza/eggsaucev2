...
from tools import database_user, PointsService, PlayerCacheService, CacheService, GlobalPlayerCache
...
class PlayerController(Cog):
    ...
    def __init__(
        self, bot: Bot, player_cache: PlayerCacheService, points_service: PointsService
    ) -> None:
        self.bot = bot
    ...
    async def donate(self, ctx: Context, amount: int, recipient: Member) -> None:
        donate_usecase = DonateUsecase(
            ctx, ctx.player_entity, amount, recipient, self.player_cache
        )
    ...
    async def steal(self, ctx: Context, target: Member) -> None:
        ...
        steal_usecase = StealUsecase(
            ctx, ctx.player_entity, target, self.player_cache, random
        )
    ...
    async def on_message(self, message: Message) -> None:
        ...
        await g.calculate_points_message(
            message.author.id, self.player_cache, self.points_service
        )
    ...
    async def on_voice_state_update(
        ...
        await g.calculate_points_voice(
            member.id, self.player_cache, self.points_service, before, after
        )
