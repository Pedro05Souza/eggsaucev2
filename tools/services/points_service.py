...
class PointsService:
    ...
    def __init__(
        self,
        cache_service: Callable[[int, int], CacheService[int, datetime]],
        max_items: int = 100,
        ...
        self.message_users_cache: CacheService[int, datetime] = cache_service(max_items, expiration_time)
        self.voice_users_cache: CacheService[int, datetime] = cache_service(max_items, expiration_time)
    ...
    def calculate_points_message(self, discord_member_id: int) -> int:
        ...
        points_calculated = self.__calculate_points_earned(self.message_users_cache, discord_member_id)
    ...
    def calculate_points_voice(self, discord_member_id: int, before: VoiceState, after: VoiceState) -> int:
        ...
        points_earnt = self.__calculate_points_earned(self.voice_users_cache, discord_member_id)
    ...
    def __calculate_points_earned(
        ...
        if use_creation_timestamp:
            ...
            last_active = cache_type.get_item(discord_member_id)
