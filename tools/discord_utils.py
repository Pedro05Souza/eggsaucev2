        ...
        if not ctx.response.is_done():
            return await ctx.response.send_message(
                embed=await embed_builder(
                    color=color, footer_text=footer_text, **kwargs
                ),
                ephemeral=ephemeral,
            )

        return await ctx.followup.send(
            embed=await embed_builder(color=color, footer_text=footer_text, **kwargs),
            ephemeral=ephemeral,
        )
...
async def embed_builder(
    color: str, footer_text: Optional[str], **kwargs
) -> Embed:
