...
class BotConfigRepository:
    ...
    async def update_bot_config(self, bot_config: BotConfigEntity) -> BotConfigEntity:
        bot_config = await BotConfig.filter(guild_id=bot_config.guild_id).update(prefix=bot_config.prefix)
        return bot_config_model_to_entity(bot_config)
    ...
    async def create_guild_config(self, discord_guild_id: int, prefix: str) -> BotConfigEntity:
        bot_config = await BotConfig.create(guild_id=discord_guild_id, prefix=prefix)
        return bot_config_model_to_entity(bot_config)
